...
# gen_file = "/hdd/common/proj/RibosomeProfiling/results/2018-04-16/CSD2_seqmagick/analysis/gene_RPKM.tab"
...
multi_orf_positions = inner_join(
  orf_positions[
...
multi_orf_positions$WithinORF = multi_orf_positions$RelPos >= 0 & multi_orf_positions$RelPosFromEnd <= 0

# Determine positions that are within at least one ORF
multi_orf_positions = aggregate(
  WithinORF ~ Sequence + strand + Position, multi_orf_positions, sum
  )

multi_orf_positions = filter(multi_orf_positions, WithinORF > 0)

# Subset gene profiles to positions that are not in multiple ORFs
gene_profiles = anti_join(gene_profiles, multi_orf_positions)
...
# ggsave("art/2018-04-19/average_gene_profile.means_and_medians.CSD2_seqmagick.b.pdf", gp, height=180/25.4, width=320/25.4)
ggsave("analysis/average_gene_profile.means_and_medians.b.pdf", gp, height=180/25.4, width=320/25.4)
